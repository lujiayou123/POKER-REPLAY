...
class Sessions(list):
    def save(self, directory):
        for session in self:
            if(os.path.exists("{}/pluribus_{}.txt".format(directory, session.label))):
                print("file {}/pluribus_{}.txt exists".format(directory, session.label))
                with open("{}/pluribus_{}.txt".format(directory, session.label), 'w') as f:
                    f.write(session.get_poker_stars_str())
            else:
                print("file {}/pluribus_{}.txt does not exist".format(directory, session.label))
                print("make empty file {}/pluribus_{}.txt".format(directory, session.label))
                os.mkdirs("{}/pluribus_{}.txt".format(directory, session.label))
                with open("{}/pluribus_{}.txt".format(directory, session.label), 'w') as f:
                    f.write(session.get_poker_stars_str())
...
if __name__ == '__main__':
    ...
    sessions = read_directory('/home/wuxuyang/Downloads/5H1AI_logs')
    sessions.save('out')
